    ...
    return (
        ...
        <Link href={`/album/${props.album.albumId}`}>
            <Grid ref={gridRef} item xs={3} sx={{ display: "flex", flexDirection: 'column', flexShrink: "0" }}>
                <Skeleton variant="rounded" sx={[loaded && { display: 'none' }, { width: width, height: width }]} />
                <CardMedia ref={imgRef} sx={[!loaded && { display: 'none' }, { borderRadius: "6%", border: "1px solid #e3e3e3" ,width: width, height: width, objectFit:"contain" }]} component='img' ></CardMedia>
                <Typography
                    variant="subtitle2"
                    sx={{
                        marginTop: "2px",
