...
当前版本
0.6.1
- 修复主页专辑卡片悬停浮动时阴影跟不上卡片的问题
- 修复全屏歌词页高亮行在暗色主题下笔画显细的问题
- 全屏歌词页窄屏适配：纵排布局，移动端控制条补上一曲/下一曲并居中
- 主页「最新上传」数量随窗口宽高联动，恒为整行并填满首屏
- 侧边栏宽度支持拖拽调整，按设备记忆

0.6.0
- 歌词
  - 全屏播放页歌词展示（按语言偏好选择，纯音乐不请求歌词）
  - 接入 LRCLIB 歌词拉取：新增拉取任务页（扫描缺失歌词、任务日志与自动刷新），音乐右键菜单支持单曲拉取
- 播放历史
  - 播放打点，新增最近播放、最常播放页面
...
0.5.1
- 首页视觉重做：推荐区栅格修复、For You 大卡、最新上传网格与悬停反馈

0.5.0
- 用户设置
  - 新增修改密码与修改绑定邮箱
- 登录
  - 适配后端登录锁定（1007）与验证码作废（1002）提示
  - 连接服务器失败时显示当前服务器，客户端内可直接修改API服务器地址并重连
  - 下线首次运行的管理账户初始化引导页（root账户改由服务端自动创建）
- 修复重新生成API代码时，请求模板回退导致的Android端基址解析失败与跨源请求登录Cookie丢失

### 0.4.x

0.4.7
 - 修复右键音乐菜单中，创建歌单对话框在输入中文时对话框消失的问题
 - 移除akari
 - 调整封面的请求大小
...
0.4.0
- 歌单
  - [x] 现已支持歌单的添加，歌单音乐的添加与删除，通过使用右键菜单即可完成操作
- 左侧面板
  - [x] 优化左侧面板的滚动条效果
  - [x] 歌单创建
...
0.3.6
- 搜索页面
  - [x] 优化交互效果、添加对largeTrack的支持
- largeTrack（多个音乐所在的同一个音轨，由CUE索引）
  - [x] 歌单页面的支持
  - [x] 首页随机音乐的支持
  - [x] 修复对时间重复处理的逻辑，优化性能
- 登录对话框
